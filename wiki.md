...
// (you replace the whole thing, not just the `XXX`)
```
You can also mix those custom SoundEvent(s) with already existing SoundEvent(s), see [Custom SoundType using already existing SoundEvent(s)](https://github.com/Minimaxi28/Minimaxi-s-KubeJS-Things/wiki/1.20.1#custom-soundtype-using-already-existing-soundevents)
